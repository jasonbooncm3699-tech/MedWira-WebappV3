...
async function checkTokenAvailability(userId, requiredCost = 1) {
    ...
    try {
        ...
        if (!profile) {
            ...
            try {
                ...
                if (newProfile.tokens < requiredCost) {
    ...
    if (profile.tokens < requiredCost) {
        console.log(`⚠️ User ${userId} has insufficient tokens (current: ${profile.tokens}, required: ${requiredCost})`);
        return { isAvailable: false, reason: "INSUFFICIENT_TOKENS" };
    }
...
async function decrementToken(userId) {
    ...
    if (selectError || !currentProfile || currentProfile.tokens <= 0) {
        console.error('❌ [DB] Cannot decrement token - user has no tokens or profile not found');
        return false;
    }

    const newTokenCount = currentProfile.tokens - 1;
    ...
    console.log(`✅ User ${userId} token decremented by 1. Remaining: ${data.tokens}`);
    return true;
}
