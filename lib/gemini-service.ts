...
IMPORTANT: Use minimal line breaks. Keep sections compact. Use bullet points (•) for lists.`;
        ...
        try {
          ...
          // Clean up the analysis text for better formatting
          comprehensiveAnalysis = this.cleanAnalysisOutput(rawAnalysis);
...
Disclaimer: This information is for educational purposes only. Consult a healthcare professional before use.`;
  ...
  /**
   * Clean up AI analysis output for better formatting
   * Removes excessive whitespace and ensures consistent formatting
   */
  private cleanAnalysisOutput(text: string): string {
    return text
      ...
      .replace(/[ \t]+$/gm, '')
      // Ensure single line breaks between sections
      .replace(/\n{3,}/g, '\n\n')
