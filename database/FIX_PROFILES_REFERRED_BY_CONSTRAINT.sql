-- =====================================================
-- FIX PROFILES REFERRED_BY CONSTRAINT - IMMEDIATE SOLUTION
-- =====================================================
-- This script fixes the foreign key constraint profiles_referred_by_fkey
...
DO $$
BEGIN
    RAISE NOTICE '🔧 STEP 2: Dropping existing constraint profiles_referred_by_fkey...';
    ...
    RAISE NOTICE '✅ STEP 2 COMPLETED: Dropped constraint profiles_referred_by_fkey';
END $$;
...
BEGIN
    ...
    RAISE NOTICE '✅ STEP 3 COMPLETED: Added constraint profiles_referred_by_fkey_cascade with ON DELETE CASCADE';
END $$;
...
BEGIN
    ...
    RAISE NOTICE '✅ Deleted % orphaned profiles records', deleted_count;
    ...
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    RAISE NOTICE '✅ Deleted % orphaned referred_by references', deleted_count;
END $$;
...
BEGIN
    ...
    SELECT EXISTS(SELECT 1 FROM public.profiles WHERE referred_by = '9638969c-b98c-4ff0-8fe0-1dbde5abdc0c') INTO referred_by_exists;
    
    RAISE NOTICE 'Before deletion - User exists: %, Profile exists: %, Referred_by exists: %', user_exists, profile_exists, referred_by_exists;
    ...
    IF user_exists THEN
        ...
        RAISE NOTICE 'After deletion - User exists: %, Profile exists: %, Referred_by exists: %', user_exists, profile_exists, referred_by_exists;
        
        IF NOT user_exists AND NOT profile_exists AND NOT referred_by_exists THEN
...
DO $$
BEGIN
    RAISE NOTICE '🎉🎉🎉 PROFILES REFERRED_BY CONSTRAINT FIX COMPLETED! 🎉🎉🎉';
    RAISE NOTICE '=====================================================';
    RAISE NOTICE '✅ STEP 1: Identified existing constraint profiles_referred_by_fkey';
    ...
    RAISE NOTICE '=====================================================';
    RAISE NOTICE 'The constraint profiles_referred_by_fkey_cascade now has ON DELETE CASCADE';
    RAISE NOTICE 'Deleting users from auth.users will automatically handle referred_by references';
    RAISE NOTICE '=====================================================';
END $$;
